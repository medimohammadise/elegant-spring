...
> Source: [https://www.ngdiagram.dev/docs/intro/styling/](https://www.ngdiagram.dev/docs/intro/styling/)
...
> **Note:** If you want to get started quickly, you can skip the detailed customization sections below. The default styling works out of the box and provides a professional appearance without any additional configuration.

> **Important:** This library relies on **global CSS variables**. Make sure the stylesheet is imported in your **global** styles file (e.g. `src/styles.scss`) as shown in the [Quick Start](https://www.ngdiagram.dev/docs/intro/quick-start).
...
> **Note:** The customization options above are **optional**.
...
```html
<!-- Node with hoverable ports (all ports highlight on node hover) -->
...
<!-- Individual hoverable port (each port highlights independently) -->
...
For cases where directives can't be used (outside node templates), apply styling manually:
